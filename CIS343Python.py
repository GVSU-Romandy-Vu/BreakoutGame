...
class Ball(Pygame.sprite.Sprite):
    ...
    def __init__(self, screen_width = 800, screen_height = 600):
        ...
        Pygame.draw.circle(self.image, (0, 0, 0), (20,20), 20, 0)
...
class Overlay(Pygame.sprite.Sprite):
    ...
    def update(self, lives, score, display):
        ...
        self.__text = font.render('Lives:'+life +"    Score:"+score, True, (0,0,0))
        #FIXME:
        screen = display
...
class Game:
    ...
    def __init__(self, screen_width = 800, screen_height = 600):
        ...
        self.__ball = Pygame.sprite.Group()
        """Variable to reference a 'Ball' object."""

        self.__paddle = Pygame.sprite.Group()
        """Variable to reference the 'Paddle' object."""

        self.__overlay = Pygame.sprite.Group()
        ...
        self.__max_score = 0
        """Max score the player can optain so it knows when to quit."""

        Pygame.mixer.music.load('alex-productions-epic-cinematic-gaming-cyberpunk-reset.mp3')
        """Music to be played when the game is running."""
        ...
        while self.__running:
            ...
            for event in events:
                ...
                #FIXME: COmplete
                    ...
                    self.__paddle.update(-1)
                elif event.type == Pygame.K_RIGHT:
                    self.__paddle.update(1)
                #Hidden keystroke to add more balls.
                elif event.type == Pygame.K_SPACE:
                    self.add_ball()
            

            #Update the logical details of the objects in the game.
            if self.__bricks.update(coll_group_1 = self.__ball):
                self.increment_score()

            if self.__ball.update(width = self.__screen_width, height = self.__screen_height, coll_group_1 = self.__paddle, coll_group_2 = self.__bricks):
            ...
            self.__ball.draw(self._screen)
            self.__paddle.draw(self._screen)
            self.__bricks.draw(self._screen)
            self.__overlay.update(self.get_lives, self.get_score, self._screen)
    ...
    def add_bricks(self, amount_per_row = 5, num_rows = 3, height_coverage = .25):
        ...
        for row in range(0, num_rows):
            ...
            for amount in range(0, amount_per_row):
